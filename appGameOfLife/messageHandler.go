...
func (app *App) MessageHandler(message *Message.Message) error {
	...
	case typeDefinitions.REQUEST_GRID_CHANGE.Name:
		...
		app.messageBroker.Send(typeDefinitions.NewWebsocketMessage([]string{}, "getGridChange", []string{message.Payload[0][0], message.Payload[1][0], Utilities.BoolToString(newGridState)}))
	case typeDefinitions.REQUEST_GRID_UNICAST.Name:
		app.mutex.Lock()
		defer app.mutex.Unlock()
		app.messageBroker.Send(typeDefinitions.NewWebsocketMessage([]string{message.Payload[0][0]}, "getGrid", []string{gridToString(app.grid)}))
	default:
