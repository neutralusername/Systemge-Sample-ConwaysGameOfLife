...
func (app *App) OnConnectHandler(connection *Websocket.Connection) {
	response, err := app.messageBrokerClient.SyncMessage(Message.New("getGridUnicast", app.name, app.randomizer.GenerateRandomString(10, Utilities.ALPHA_NUMERIC), connection.Id))
	if err != nil {
